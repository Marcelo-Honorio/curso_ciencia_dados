...
data: "[bit.ly/introducao-ds](https://bit.ly/introducao-ds)"
format: 
  revealjs:
...
::: incremental
-   Por que aprender um linguagem de programação?
-   Por que o R?
...
-   Aprender a programar é muito importante quando desejamos entender os dados! Mais: é importante para que possamos intepretar esses dados; para que possamos transformá-los em informação!
...
-   Possui inúmeros pacotes (mais de 10 mil), com vantagem para a aplicação da Estatística Avançada e uma vasta comunidade de suporte, além de fortes capacidades voltadas ao Data Science.
-   CRAN (Comprehensive R Archive Network) é o repositório principal da linguagem R em que cada usuário pode contribuir como novos pacotes (que são coleções de funções em R como código compilado). Esses pacotes podem ser facilmente instalados com um linha de código.

## Sobre o R

-   leituras recomendadas para quem inda não conhece a linguagem R e deseja se aprofundar:
...
-   **Objetos** são maneiras simples de acessar algo que foi salvo na memória da máquina. Pode ser um valor, uma palvras, uma ou mais variáveis, uma URL, uma base de dados amostral ou populacional, etc. **No R TUDO** é um objeto! Cada um desses objetos possui uma classe!
...
-   Logical <br> [`5 > 1`/`TRUE`]{style="color:#00589b"}
-   String <br> `"Marcelo"`{style="color:#00589b"}
-   Factor <br> `"Homem"/"Mulher"`{style="color:green"}
:::
...
- `if`{style="color:#00589b"} e `else`{style="color:#00589b"} servem para executarmos um código apenas se uma condição (teste lógico) for satisfeita

- `for`{style="color:#00589b"} repetir uma mesma tarefa para um conjunto de valores diferentes

- `while`{style="color:#00589b"} repetir uma mesma tarefa enquanto a condições for respeitada
...
uma coleção de pacotes R cuidadosamente desenhados para atuarem no workflow comum da ciência de dados:
...
- `filter`{style="color:#00589b"}: manter ou descartar linhas 
- `select`{style="color:#00589b"}: manter ou descartar colunas
- `count`{style="color:#00589b"}: contabiliza os dados por variável
- `arrange`{style="color:#00589b"}: ordenar o dataset por vaiável
- `mutate`{style="color:#00589b"}: criar nova vaiável
- `group_by`{style="color:#00589b"}/`summarize`{style="color:#00589b"}: agregar dados
- `*_join`{style="color:#00589b"}: unir conjunto de dados
...
`filter()`{style="color:#00589b"} manter ou discartar linhas
...
`count`{style="color:#00589b"} contabiliza os dados de um determinada variável
...
`mutate`{style="color:#00589b"} criar nova vaiável (com um `=`{style="color:#00589b"})
```{r,  echo=TRUE, eval=FALSE}
dataset |> 
  mutate(new = "hello!")
```
...
`(condição, valor se TRUE, valor se TRUE)`{style="color:#00589b"}
:::
```{r,  echo=TRUE, eval=FALSE}
dataset |> 
...
`case_when()`{style="color:#00589b"} para múltiplas condições
```{r, echo=TRUE, eval=FALSE}
faculty %>% 
...
carrega oito pacotes principais quando invocado e também agrupa outro pacotes na instalação
:::
